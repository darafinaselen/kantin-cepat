...
public class MenuPanel extends JPanel {
    ...
    public MenuPanel(ClientSocket socket) {
        ...
        btnDel.addActionListener(e -> {
            ...
            if (selectedRow >= 0) {
                // Ambil ID dari kolom ke-1 (karena kolom 0 adalah Foto)
                String id = tableModel.getValueAt(selectedRow, 1).toString();
                String namaMenu = tableModel.getValueAt(selectedRow, 2).toString();
                ...
                if (confirm == JOptionPane.YES_OPTION) {
                    ...
                    new Thread(() -> {
                        String response = socketService.sendRequest("DELETE_MENU;" + id);
                        
                        SwingUtilities.invokeLater(() -> {
                            if ("SUCCESS".equalsIgnoreCase(response)) {
                                tableModel.removeRow(selectedRow); // Hapus dari GUI
                                clearForm(); // Bersihkan form
                                JOptionPane.showMessageDialog(this, "Data Berhasil Dihapus!");
                            } else {
                                JOptionPane.showMessageDialog(this, "Gagal menghapus: " + response);
                            }
                        });
                    }).start();
                }
